...
/**
 * Fires once, right after a new cashier account is created — welcomes
 * them + hands over their login email/password via Gmail SMTP (see
 * mail_config.php for credentials). Returns true/false so the caller
 * can note a failure without blocking account creation itself.
 */
function send_cashier_welcome_email(string $email, string $fullName, string $password, ?string $shiftStart, ?string $shiftEnd): bool {
    $firstName = trim(explode(' ', $fullName)[0] ?? $fullName);

    $shiftLine = ($shiftStart && $shiftEnd)
        ? '<p><strong>Your assigned shift:</strong> ' . htmlspecialchars(date('g:i A', strtotime($shiftStart))) . ' - ' . htmlspecialchars(date('g:i A', strtotime($shiftEnd))) . '</p>'
        : '';
    ...
    try {
        ...
        $mail->isHTML(true);
        $mail->Subject = 'Welcome to RAM-YUM STORE!';
        $mail->Body    = "
            <p>Hi " . htmlspecialchars($firstName) . ",</p>
            <p>Welcome to the RAM-YUM STORE team! Your cashier account is ready.</p>
            <p><strong>Login email:</strong> " . htmlspecialchars($email) . "<br>
            <strong>Temporary password:</strong> " . htmlspecialchars($password) . "</p>
            {$shiftLine}
            <p>Please log in and change your password on your first shift.</p>
            <p>Glad to have you with us!<br>— RAM-YUM STORE Management</p>
        ";
        $mail->AltBody = "Hi {$firstName}, welcome to RAM-YUM STORE! Login email: {$email} / Temporary password: {$password}. Please change your password on your first shift.";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'create_user') {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($password) < 6 || !in_array($role, ['admin', 'manager', 'cashier'], true)) {
            ...
            // New cashier accounts get a welcome email with their login
            // details. Doesn't block account creation if send fails —
            // just note it in the flash so the admin knows to follow up.
            if ($_SESSION['flash']['type'] === 'success' && $role === 'cashier') {
                $sent = send_cashier_welcome_email($email, $fullName, $password, $shiftStartV, $shiftEndV);
                if (!$sent) {
                    $_SESSION['flash']['msg'] .= ' (Could not send welcome email — check mail server config.)';
                }
            }
        }
    }
