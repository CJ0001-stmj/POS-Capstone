...
// Full line-item breakdown (name/qty/unit price/line total) per order and
// reservation, for the "view products" preview modal. Grouped in PHP so
// this page still works fine even with an empty queue.
$orderItemsById = [];
if ($pendingOrders) {
    $ids = array_map(fn($o) => (int) $o['id'], $pendingOrders);
    $ph = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $db->prepare("SELECT pending_order_id, product_name, unit_price, quantity, line_total
                           FROM pending_order_items WHERE pending_order_id IN ($ph)");
    $stmt->execute($ids);
    foreach ($stmt->fetchAll() as $row) {
        $orderItemsById[$row['pending_order_id']][] = $row;
    }
}
$resvItemsById = [];
if ($reservations) {
    ...
    $ph = implode(',', array_fill(0, count($ids), '?'));
    $stmt = $db->prepare("SELECT reservation_id, product_name, unit_price, quantity, line_total
                           FROM reservation_items WHERE reservation_id IN ($ph)");
    $stmt->execute($ids);
    foreach ($stmt->fetchAll() as $row) {
        $resvItemsById[$row['reservation_id']][] = $row;
    }
}
?>
<!DOCTYPE html>
                ...
                <div class="panel history-panel">
                    ...
                    <table class="transactions-table queue-table">
                        ...
                        <?php else: foreach ($pendingOrders as $o): ?>
                            <tr class="queue-row" data-type="order" data-id="<?= (int)$o['id'] ?>"
                                data-total="<?= (float)$o['total'] ?>"
                                data-subtotal="<?= (float)$o['subtotal'] ?>"
                                data-discount="<?= (float)$o['discount'] ?>"
                                data-promotion="<?= htmlspecialchars($o['promotion_name'] ?? '') ?>"
                                data-ref="<?= htmlspecialchars($o['order_no']) ?>"
                                data-items='<?= htmlspecialchars(json_encode($orderItemsById[$o['id']] ?? []), ENT_QUOTES) ?>'>
                                ...
                                <td class="pr-row-actions">
                                    <button type="button" class="pos-btn-secondary queue-view-btn" style="padding:7px 13px; font-size:0.78rem;"><i class="fa-solid fa-eye"></i> View</button>
                ...
                <div class="panel history-panel">
                    ...
                    <table class="transactions-table queue-table">
                        ...
                        <?php else: foreach ($reservations as $r): ?>
                            <tr class="queue-row" data-type="reservation" data-id="<?= (int)$r['id'] ?>"
                                data-total="<?= (float)$r['total'] ?>"
                                data-subtotal="<?= (float)$r['subtotal'] ?>"
                                data-discount="<?= (float)$r['discount'] ?>"
                                data-promotion="<?= htmlspecialchars($r['promotion_name'] ?? '') ?>"
                                data-ref="<?= htmlspecialchars($r['reservation_no']) ?>"
                                data-items='<?= htmlspecialchars(json_encode($resvItemsById[$r['id']] ?? []), ENT_QUOTES) ?>'>
...
<div class="pos-modal-overlay" id="viewOverlay">
    <div class="pos-modal">
        <h3><i class="fa-solid fa-eye"></i> Order Contents <span id="viewRefLabel"></span></h3>
        <p class="pos-modal-sub">Review the products before processing this.</p>
        <div class="table-scroll">
        <table class="transactions-table" id="viewItemsTable">
            <thead>
        ...
        <div class="confirm-reservation-summary">
            <div class="r-row"><span>Subtotal</span><strong id="viewSubtotalDisplay">₱0.00</strong></div>
            <div class="r-row" id="viewDiscountRow" style="display:none;"><span id="viewPromoLabel">Discount</span><strong id="viewDiscountDisplay">-₱0.00</strong></div>
            <div class="r-row"><span>Total</span><strong id="viewTotalDisplay">₱0.00</strong></div>
        </div>
        <div class="pos-modal-actions">
            <button type="button" class="pos-btn-secondary" id="viewCloseBtn">Close</button>
        </div>
    </div>
</div>
...
<div class="pos-modal-overlay" id="paymentOverlay">
    <div class="pos-modal">
        ...
        <div class="confirm-reservation-summary">
            <div class="r-row"><span>Subtotal</span><strong id="paymentSubtotalDisplay">₱0.00</strong></div>
            <div class="r-row" id="paymentDiscountRow" style="display:none;"><span id="paymentPromoLabel">Discount</span><strong id="paymentDiscountDisplay">-₱0.00</strong></div>
...
<div class="pos-modal-overlay" id="orderReceiptOverlay">
    <div class="pos-modal pos-receipt-modal">
        ...
        <div class="pos-modal-actions">
            ...
            <button type="button" class="pos-btn-secondary" id="downloadReceiptBtn"><i class="fa-solid fa-download"></i> Download</button>
            <button type="button" class="pos-btn-primary" id="closeReceiptBtn"><i class="fa-solid fa-check"></i> Done</button>
        </div>
    </div>
</div>

<script src="/orm/orders.js"></script>
